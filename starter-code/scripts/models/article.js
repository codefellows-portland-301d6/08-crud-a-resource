(function(module) {
  ...
  Article.createTable = function() {
    webDB.execute(
      'CREATE TABLE IF NOT EXISTS pleasework (id INTEGER PRIMARY KEY, title VARCHAR, category VARCHAR, author VARCHAR, authorUrl VARCHAR, publishedOn VARCHAR, body VARCHAR);', // DONE: What SQL command do we run here inside these quotes?
      function() {
  ...
  Article.fetchAll = function(nextFunction) {
    ...
    webDB.execute('SELECT * FROM pleasework;', function(rows) { // DONE: fill these quotes to query our table.
      if (rows.length) {
        /* DONE:
        ...
        $.getJSON('/data/hackerIpsum.json', function(responseData) {
          ...
          responseData.forEach(function(obj) {
            ...
            webDB.execute([{
              'sql': 'INSERT INTO pleasework (title, category, author, authorURL, publishedOn, body) VALUES (?, ?, ?, ?, ?, ?);',
              'data': [article.title, article.category, article.author, article.authorURL, article.publishedOn, article.body]
            }]);
          });
          // Now get ALL the records out the DB, with their database IDs:
          webDB.execute('SELECT * FROM pleasework;', function(rows) { // DONE: select our now full table
            // DONE:
      ...
      [
        {
          ...
          'sql': 'INSERT INTO pleasework (title, category, author, authorURL, publishedOn, body) VALUES (?, ?, ?, ?, ?, ?);', // <----- complete our SQL command here, inside the quotes.
