(function(module) {
  ...
  Article.createTable = function() {
    webDB.execute(
      // DONE: What SQL command do we run here inside these quotes?
      'CREATE TABLE IF NOT EXISTS articles (id INTEGER PRIMARY KEY, title VARCHAR, author VARCHAR, author_url VARCHAR, category VARCHAR, published_on DATE, body TEXT)',
      function() {
      ...
      [
        {
          ...
          'sql': 'INSERT INTO articles (title, author, author_url, category, published_on, body) VALUES (?, ?, ?, ?, ?, ?)', // <----- complete our SQL command here, inside the quotes.
